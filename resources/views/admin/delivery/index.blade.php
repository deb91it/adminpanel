                ...
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text">
                        ...
                        <div class="m-portlet__head-tools">
                            ...
                            <button type="button" class="m-btn btn btn-outline-info export-file" name="csv" title="Csv">
                                <i class="fa fa-file-o"> &nbsp;Download CSV</i>
                            </button>
...
{{--                            </div>--}}
                            ...
                            <div class="col-md-2 {!! $errors->has('assign_to') ? 'has-error' : ''  !!}" id="div_assign">
                                <label id="selected-rider">Assign To <span class="text-danger">*</span></label>
                                <select class="form-control m-input" name="assign_to" id="assign_to">
                                </select>
                                {!! $errors->first('assign_to', '<label class="error_txt_size">:message</label>') !!}
                            </div>
                        ...
                        <table id="Deliverys-datatable" class="table table-sm m-table m-table--head-bg-brand table-striped table-hover">
                            <thead class="">
                                <tr>
                                    ...
                                    <th>Delivered Plan Name</th>
                                    <th>Returned Plan Name</th>
                                    <th>Charge</th>
                                    <th>COD</th>
                                    <th>Total Charge</th>
                            ...
                            <div class="m-section__content">
                                <div class="table-responsive">
                                    <table id="payment-receive-details" class="table table-sm m-table m-table--head-bg-brand table-striped table-hover custom_table_responsive">
                                        ...
                                        <th></th>
                                        <th></th>
                                        <th><input type="text" readonly class="form-control" id="totalCollectedAmount"></th>
                                        <th class="text-center"><input type="text" readonly class="form-control" id="totalReceivedAmount"></th>
                                        <th></th>
                                        </tfoot>
                                    </table>
                                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            var table = $('#Deliverys-datatable').DataTable({
                ...
                dom : 'l<"#date-filter"><"#sorting-filter"><"#hub-filter"><"#rider-filter"><"#entry-filter">frtip',
                ajax: {
                    ...
                    data: function( d ) {
                        ...
                        d.merchant_id = "{{ !empty($_GET['merchant_id']) ? $_GET['merchant_id'] : ''}}";
                    }
                },
                columns: [
                    {
                    ...
                    },
                    {
                        ...
                        searchable: true,
                        render: function (data, type, row, meta) {
                            let deliId = [row.deli_id]
                            let link = 'delivery/'+JSON.stringify(deliId)+'/invoice-pdf';
                            return '<a href="'+link+'" target="_blank">'+row.consignment_id+'</a>';
                        }
                    },
                    {
                        data: 'full_name',
                        ...
                        searchable: true,
                        render: function (data, type, row, meta) {
                            return row.business_name+'<br><small style="color: #716ACA">'+row.full_name+'</small>'+'<br><small style="color: #02768E">'+row.merchant_mobile_no+'</small>';
                        }
                    ...
                    },
                    {
                        ...
                        render: function ( data, type, row ) {
                            return row.recipient_name+'<br><small style="color: #287706;">'+row.recipient_address+'</small>'+'<br><small style="color: #716aca;">'+row.recipient_email+'</small>'+'<br><small style="color: #02768E;">'+row.recipient_number+'</small>';
                        }
                    },
                    {
                        data: 'zone_name',
                    ...
                    {
                        data: 'returned_plan_name',
                        name: 'returned_plan_name',
                        searchable: false
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        render: function ( data, type, row ) {
                            return parseFloat(row.charge)+ parseFloat(row.cod_charge)
                        }
                    ...
                    {
                        data: 'entry_created_by',
                        name: 'entry_created_by',
                        searchable: false
                    },
                    {
                        data: 'entry_updated_by',
            ...
            $('#sorting-filter').addClass('col-lg-4 col-md-4 col-sm-4');
            $('#hub-filter').addClass('col-lg-3 col-md-3 col-sm-3');
            $('#rider-filter').addClass('col-lg-4 col-md-4 col-sm-4');
            $('#entry-filter').addClass('col-lg-5 col-md-5 col-sm-5');
            ...
            //Filter by Rider
            var rider_select_html =
                '<div class="form-group m-form__group clearfix">' +
                '<div class="input-group pull-right" >' +
                // '<label>select role</label>' +
                '<select id="sorting_rider" class="form-control m_selectpicker1"><option value="">Filter by Rider</option><?php
                    if(!empty($getRiders)){
            ...
            $('#rider-filter').append(rider_select_html);
            $('#sorting_rider').change(function() {
                var rider = $(this).val();
                table.columns(15).search(rider).draw();
            });

            //Filter by Data Entry
            var entry_select_html =
                '<div class="form-group m-form__group clearfix">' +
                '<div class="input-group pull-right" >' +
                // '<label>select role</label>' +
                '<select id="sorting_entry" class="form-control m_selectpicker1"><option value="">Filter by Data Entry</option><?php
            ...
            $('#entry-filter').append(entry_select_html);
            $('#sorting_entry').change(function() {
                var data_entry = $(this).val();
                table.columns(18).search(data_entry).draw();
            });
            <?php } ?>
            var consignment = [];
            $("#barcode_scanner").change(function () {
                var parcel = $(this).val().split("-");
                if(parcel[0] == 'PROVATI' && parcel[1].length == 8)
                {
            ...
            $('#Deliverys-datatable_filter label:first-child input').attr('id', 'delivery-search-filter');
    ...
    <script type="text/javascript">
        ...
        $(document).on('click','.export-file',function(e)
        {
            ...
            var queryString = $("#delivery-search-filter").val();
            var status = $("#sorting_role").val();
            var createdBy = $("#sorting_entry").val();

            $.ajax({
                type: "POST",
                url: "{!! route('admin.deliverys.export') !!}",
                data: {"_token": "{{ csrf_token() }}", "export_type": exportType, "start_date": startDate, "end_date": endDate, "query_string": queryString, "status": status, "created_by": createdBy},
                cache: false,
                beforeSend: function(){
        ...
        $(document).on('keyup','#delivery-search-filter',function (e)
        {
        ...
        $("#flag_status_id").change(function () {
            ...
            if ($(this).val() == 6 || $(this).val() == 7 || $(this).val() == 8 || $(this).val() == 12 || $(this).val() == 16){
                $.ajax
                ({
                    type: "POST",
                    ...
                    success: function(res)
                    {
                        ...
                        $.each(res.response, function (key, val) {
                            ...
                            content +='<tr>';
                            content += '<td><input onclick="summationOfCheckedVal()" type="checkbox" class="checkbox modal-select-checkbox" '+is_approve+' class="m-checkbox m-checkbox--bold m-checkbox--state-brand" name="deliveried_id[]" value="'+val['deli_id']+'">  </td>';
                            content += '<td>' + val['consignment_id'] + '</td>';
                            content += '<td>' + val['delivery_date'] + '</td>';
                            content += '<td style="color: '+val['color_code']+'">' + val['flag_text'] + '</td>';
                            content += '<td id="amount_to_be_collected'+val['deli_id']+'">' + val['amount_to_be_collected'] + '</td>';
                            content += '<td><input onchange="summationOfCheckedVal()" id="received_amount'+val['deli_id']+'" type="text" class="form-control" value="' + val['amount_to_be_collected'] + '" name="received_amount'+val['deli_id']+'">';
        ...
        function summationOfCheckedVal()
        {
            ...
            $("#payment-receive-details-body input[type=checkbox]:checked").each(function () {
                // console.log($(this).val())
                summationOfCollectedAmount = summationOfCollectedAmount + parseFloat($("#amount_to_be_collected"+$(this).val()).text());
                summationOfReceivedAmount = summationOfReceivedAmount + parseFloat($("#received_amount"+$(this).val()).val());
            });
            ...
            $("#totalCollectedAmount").val(summationOfCollectedAmount);
            $("#totalReceivedAmount").val(summationOfReceivedAmount);
        }

        function deleteDelivery(id) {
            if (confirm("Are you sure?")) {
                let location = window.location.replace("/admin/delivery/"+id+"/delete");
            }
            return false;
        }
