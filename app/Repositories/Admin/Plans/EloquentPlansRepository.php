<?php namespace App\Repositories\Admin\Plans;
...
class EloquentPlansRepository implements PlansRepository
{
    protected $plan;

    function __construct(Plans $plan)
    {
        $this->plan = $plan;
    }
    ...
    public function update($input, $id)
    {
        ...
        if ($plan->plan_code == 0) {
            $plan->plan_code   = strtoupper(getUniquePlanCode(8));
        }
        $plan->plan_name   = $input['plan_name'];
        $plan->plan_type   = $input['plan_type'];
        $plan->charge   = $input['charge'];
...
//                DB::raw('CONCAT(merch.first_name, " ", merch.last_name) AS full_name')
        ...
        return Datatables::of($query)
            ...
            ->addColumn('action_col', function ($user) {
                return '
                    <a href="'.route('admin.plan.edit',array($user->id)).'" class="btn btn-sm m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit"><i class="fa fa-edit"></i></a>
                    <a href="'.route('admin.plan.delete',array($user->id)).'" class="btn btn-sm m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Delete"><i class="fa fa-trash"></i></a>
                    ';
            })
            ->make(true);
    }
...
//            DB::raw('CONCAT(merch.first_name, " ", merch.last_name) AS full_name'),
    ...
    public function exportFile($request)
    {
        ...
        $query = DB::table('members as m')
            ...
            ->join('Plans as merch', 'merch.member_id', '=', 'm.id')
    ...
    {
        return  DB::table('Plans as merch')
