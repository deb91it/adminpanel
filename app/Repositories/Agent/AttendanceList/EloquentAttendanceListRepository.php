...
class EloquentAttendanceListRepository implements AttendanceListRepository
{
    protected $agent;
    protected $role;
    ...
    {
        $role_data = $request->get('columns')[0]['search']['value'];
        $date_range = $request->get('columns')[4]['search']['value'];
        if ($date_range != '') {
        ...
        $query=  DB::table('agents as ag')
            ->select(
                ...
                'ag.designation','ag.id','attend.attendance_date','attend.c_time','attend.o_time','attend.current_status',
            ...
            ->leftjoin(DB::raw("(select employee_id,
                    ...
                    from sr_attendence 
                    where attendance_date between "."'".$from."'"." AND "."'".$to."'".") as attend"), 'ag.id', '=', 'attend.employee_id')
            ->where([ 'ag.company_id' => get_agent_company_id() ]);
        if (!empty($role_data)) {
            $query = $query->where('rm.role_id', $role_data);
        }
        ...
        return Datatables::of($query)
            ...
            ->filterColumn('designation', function($query, $keyword) {
                $query->whereRaw("ag.designation like ?", ["%{$keyword}%"]);
            })
            ->addColumn('action_col', function ($user) {
                return '
                    ...
                    <a onclick="viewAttendanceList('.$user->id.')" class="btn btn-sm m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="View Details"><i class="fa fa-expand"></i></a>
